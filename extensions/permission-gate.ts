...
const DANGEROUS_BASH: { pattern: string; label: string }[] = [
	...
	// PowerShell 命令（仅当命令以 powershell 开头时检测，避免 grep 搜索文本误报）
	{ pattern: "^\\s*powershell.*\\bRemove-Item(\\s|$)", label: "PowerShell 删除 (Remove-Item)" },
	{ pattern: "^\\s*powershell.*\\brm\\s+-recurse\\b", label: "PowerShell 递归删除 (rm -recurse)" },
	{ pattern: "^\\s*powershell.*\\bdel\\s+/f\\b", label: "PowerShell 强制删除 (del /f)" },
	{ pattern: "^\\s*powershell.*\\brd\\s+/s\\b", label: "PowerShell 递归删除 (rd /s)" },
	{ pattern: "^\\s*powershell.*\\brmdir\\s+/s\\b", label: "PowerShell 递归删除 (rmdir /s)" },
	{ pattern: "^\\s*powershell.*\\bFormat-Volume\\b", label: "格式化磁盘 (Format-Volume)" },
	{ pattern: "^\\s*powershell.*\\bClear-Content\\b", label: "清空文件 (Clear-Content)" },
	{ pattern: "^\\s*powershell.*\\bStop-Computer\\b", label: "关闭计算机 (Stop-Computer)" },
	{ pattern: "^\\s*powershell.*\\bRestart-Computer\\b", label: "重启计算机 (Restart-Computer)" },
	{ pattern: "^\\s*powershell.*\\bSet-ExecutionPolicy\\b", label: "修改执行策略 (Set-ExecutionPolicy)" },
